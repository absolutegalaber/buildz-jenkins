...
"""
        ...
        String response = script.sh(
                returnStdout: true,
                script: """curl -X POST -H "Content-Type: application/json" --data @nextVersionRequest.json ${script.env.BUILDZ_API}/api/v1/build-numbers/next"""
        )
...
"""
        ...
        String response = script.sh(
                returnStdout: true,
                script: """curl -X POST -H "Content-Type: application/json" --data @createBuildRequest.json ${script.env.BUILDZ_API}/api/v1/builds/create"""
        )
        //read the returned json into a string
    ...
    static void addBuildzLabel(script, Long buildId, String key, String value) {
        script.writeFile file: 'addLabelsRequest.json', text: """{"${key}": "${value}"}"""
        script.sh(
                script: """curl -X POST -H "Content-Type: application/json" --data @addLabelsRequest.json ${script.env.BUILDZ_API}/api/v1/builds/add-labels/${buildId}"""
        )
    }
}